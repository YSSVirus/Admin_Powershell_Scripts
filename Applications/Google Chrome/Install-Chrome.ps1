...
function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Install-Chrome.log"
    )
...
if ((Test-Path "C:\YSS\Logs\Install-Chrome.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Install-Chrome.log" -Force -ErrorAction SilentlyContinue
}

$application_installed = install-verify "Chrome"

#if ($application_installed -eq $true) {
#    message-log "Chrome is already installed - Exiting" -message_type "error"
#    exit 0 
#}
#else {
#    message-log "Chrome is not installed, continuing"
#}

$user_role_admin = admin-check

if ($user_role_admin -and $attempt_install_machine_wide) {
    $installer_url = "https://dl.google.com/chrome/install/googlechromestandaloneenterprise64.msi"
    $installer_name = "chrome.msi"
}
else {
    $installer_url = "https://dl.google.com/tag/s/appguid%3D%7B8A69D345-D564-463C-AFF1-A69D9E530F96%7D%26iid%3D%7BEE7717A6-1279-5B62-7B81-146818152BD4%7D%26lang%3Den%26browser%3D4%26usagestats%3D1%26appname%3DGoogle%2520Chrome%26needsadmin%3Dprefers%26ap%3D-arch_x64-statsdef_1%26installdataindex%3Dempty/update2/installers/ChromeSetup.exe"
    $installer_name = "chrome.exe"
}
...
if ((Test-Path $file_download_path) -eq $false) {
    message-log "Chrome installer could not be detected after attempted download (Location: $file_download_path) (Installer url: $installer_url)" -message_type "error"
    exit 1
}
else {
    message-log "Chrome downloaded"
}
...
if ($user_role_admin -and $attempt_install_machine_wide) {
    ...
    $process_current_list = process-monitornew -process_name "Google Updater" -scan_only $true
    Start-Process "$file_download_path" -ArgumentList "/silent"
    $process_current_id = process-monitornew -process_name "Google Updater" -scan_only $false -process_id_info_old $process_current_list
}
...
if ($application_installed -eq $true) {
    message-log "Chrome is installed (Version: $application_version)" -message_type "success"
    exit 0 
}
else {
    message-log "Chrome could not be detected after attempted install" -message_type "error"
    exit 1
}