...
function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Install-Bitwarden.log"
    )
...
if ((Test-Path "C:\YSS\Logs\Install-Bitwarden.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Install-Bitwarden.log" -Force -ErrorAction SilentlyContinue
}

$application_installed, $application_version = install-verify "Bitwarden"

if ($application_installed -eq $true) {
    message-log "Bitwarden is already installed - Exiting" -message_type "error"
    exit 0 
}
else {
    message-log "Bitwarden is not installed, continuing"
}

$user_role_admin = admin-check

$installer_url = "https://bitwarden.com/download/?app=desktop&platform=windows&variant=exe"
$installer_name = "bitwarden.exe"
...
if ((Test-Path $file_download_path) -eq $false) {
    message-log "Bitwarden installer could not be detected after attempted download (Location: $file_download_path) (Installer url: $installer_url)" -message_type "error"
    exit 1
}
else {
    message-log "Bitwarden downloaded"
}

# Install
if ($user_role_admin -and $attempt_install_machine_wide) {
    $process_current_list = process-monitornew -process_name "bitwarden" -scan_only $true
    Start-Process "$file_download_path" -ArgumentList "/allusers /S"
    $process_current_id = process-monitornew -process_name "bitwarden" -scan_only $false -process_id_info_old $process_current_list
}
else {
    $process_current_list = process-monitornew -process_name "bitwarden" -scan_only $true
    Start-Process "$file_download_path" -ArgumentList "/currentuser /S"
    $process_current_id = process-monitornew -process_name "bitwarden" -scan_only $false -process_id_info_old $process_current_list
}
...
if ($application_installed -eq $true) {
    message-log "Bitwarden is installed (Version: $application_version)" -message_type "success"
    exit 0 
}
else {
    message-log "Bitwarden could not be detected after attempted install" -message_type "error"
    exit 1
}