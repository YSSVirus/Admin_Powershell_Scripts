...
function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Install-Adobe.log"
    )
...
if ((Test-Path "C:\YSS\Logs\Install-Adobe.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Install-Adobe.log" -Force -ErrorAction SilentlyContinue
}

$application_installed, $application_version = install-verify "Adobe"

if ($application_installed -eq $true) {
    message-log "Adobe is already installed - Exiting" -message_type "error"
    exit 0 
}
else {
    message-log "Adobe is not installed, continuing"
}
...
$installer_url = "https://admdownload.adobe.com/rdcm/installers/live/readerdc64A_ha_crd_hrmd_install.exe?filename=Reader_en_install.exe"
$installer_name = "Adobe.exe"
...
if ((Test-Path $file_download_path) -eq $false) {
    message-log "Adobe installer could not be detected after attempted download (Location: $file_download_path) (Installer url: $installer_url)" -message_type "error"
    exit 1
}
else {
    message-log "Adobe downloaded"
}

# Install
if ($user_role_admin -and $attempt_install_machine_wide) {
    $process_current_list = process-monitornew -process_name "adobe" -scan_only $true
    Start-Process "$file_download_path" -ArgumentList "/S"
    $process_current_id = process-monitornew -process_name "adobe" -scan_only $false -process_id_info_old $process_current_list
}
...
if ($application_installed -eq $true) {
    message-log "Adobe is installed (Version: $application_version)" -message_type "success"
    exit 0 
}
else {
    message-log "Adobe could not be detected after attempted install" -message_type "error"
    exit 1
}