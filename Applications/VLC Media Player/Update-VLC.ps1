...
function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Update-VLC.log"
    )
...
function Resolve-VlcMirror {
    param([Parameter(Mandatory)][string]$Url)
    $page = Invoke-WebRequest -Uri $Url -UseBasicParsing
    $m = [regex]::Match($page.Content, '(?i)<meta[^>]*http-equiv=["'']?refresh["'']?[^>]*content=["''][^;]*;\s*URL=[''"]?(?<u>[^''">]+)')
    if (-not $m.Success) { 
        message-log "No mirror redirect found on $Url — page markup may have changed" -message_type "error"
        exit 1
    }
    $m.Groups['u'].Value
}
...
if ((Test-Path "C:\YSS\Logs\Update-VLC.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Update-VLC.log" -Force -ErrorAction SilentlyContinue
}

$user_role_admin = admin-check

if ($user_role_admin -eq $false) {
    message-log "User is not admin, admin is required for this installer" -message_type "error"
    exit 1
}
...
if ($application_installed -eq $false) {
    message-log "VLC is not installed, we can not update it - Exiting" -message_type "error"
    exit 0 
}
else {
    message-log "VLC is installed, continuing"
}
...
if ($application_installed -eq $true) {
    ...
    message-log "VLC could not be detected after attempted install" -message_type "error"
    exit 1
}