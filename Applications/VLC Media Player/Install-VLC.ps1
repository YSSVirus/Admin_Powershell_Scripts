function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Install-VLC.log"
    )
...
if ((Test-Path "C:\YSS\Logs\Install-VLC.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Install-VLC.log" -Force -ErrorAction SilentlyContinue
}
...
if ($application_installed -eq $false) {
    ...
    message-log "VLC is installed, exiting" -message_type "info"
    exit 0
}
