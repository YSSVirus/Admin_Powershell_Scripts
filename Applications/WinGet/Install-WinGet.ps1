...
function message-log() {
    param (
        ...
        [string] $file_log = "C:\YSS\Logs\Install-Winget.log"
    )
...
if ((Test-Path "C:\YSS\Logs\Install-Winget.log") -eq $true) {
    Remove-Item "C:\YSS\Logs\Install-Winget.log" -Force -ErrorAction SilentlyContinue
}

$application_installed, $application_version = install-verify "Winget"

if ($application_installed -eq $true) {
    message-log "Winget is already installed - Exiting" -message_type "error"
    exit 0 
}
else {
    message-log "Winget is not installed, continuing"
}

$installer_url = "https://api.github.com/repos/microsoft/winget-cli/releases/latest"
$installer_url = (Invoke-WebRequest -Uri $URL -UseBasicParsing).Content | ConvertFrom-Json |
...
if ((Test-Path $file_download_path) -eq $false) {
    message-log "Winget installer could not be detected after attempted download (Location: $file_download_path) (Installer url: $installer_url)" -message_type "error"
    exit 1
}
else {
    message-log "Winget downloaded"
}
...
if ($application_installed -eq $true) {
    message-log "Winget is installed (Version: $application_version)" -message_type "success"
    exit 0 
}
else {
    message-log "Winget could not be detected after attempted install" -message_type "error"
    exit 1
}